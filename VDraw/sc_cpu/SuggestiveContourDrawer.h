...
private:
	bool fading_;
	float sc_thresh_;
	void construct_sc_segments(Model *m, int vec0, int vec1, int vec2, float fade_factor, int face);
	void find_sc_segments(Model* m, vec camera_position, float fade_factor);
public:
	virtual void init();
	std::vector<std::vector<vec>> scSegments;
	std::vector<int> scSegmentationIndices;
	std::vector<int> scFaces;
