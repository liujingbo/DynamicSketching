 ...
 */
EdgeContourDrawer::EdgeContourDrawer(vec color = vec(0,0,0), float linewidth = 1.0): LineDrawer(color,linewidth)
{
	//nothing left to do
}
void EdgeContourDrawer::init(){
...
void EdgeContourDrawer::draw(Model* m, vec camera_position)
{
	if(this->isCompute()){
		...
		// m->edgeContour = this->get_drawbuffer_vertices_();
		m->edgeVertices = this->edgeVertices;
		this->edgeVertices.clear(); // clear, to avoid accumulation across frames
	...
	for(unsigned int i =0; i < faces.size(); i++){
		if(to_camera(m,i,camera_position)){
			...
			if (!to_camera(m,edge[i][0],camera_position)){
				...
				this->edgeVertices.push_back(faces[i][1]);
				this->edgeVertices.push_back(faces[i][2]);
			}
			if (!to_camera(m,edge[i][1],camera_position)){
				drawbuffer_vertices_.push_back(vertices[faces[i][0]]);
				drawbuffer_vertices_.push_back(vertices[faces[i][2]]);
				this->edgeVertices.push_back(faces[i][0]);
				this->edgeVertices.push_back(faces[i][2]);
			}
