...
void SuggestiveContourDrawer::init(){
	scSegments.clear();
}
/**
...
void SuggestiveContourDrawer::draw(Model* m, vec camera_position){
	if(this->isCompute()){
		...
		// tell curve manager of the edge contours
		m->suggestiveContour = scSegments;
		m->scSegmentationIndices = this->scSegmentationIndices;
		m->scFaces = this->scFaces;
		scSegments.clear();
		this->scSegmentationIndices.clear();
		this->scFaces.clear();
 ...
 */
void SuggestiveContourDrawer::construct_sc_segments(Model *m, int vec0, int vec1, int vec2, float fade_factor, int face)
{
	// aliases
	...
	if(zero_num){ // if the dwKr dips below zero, first segment ends here. or vice versa, it starts here
		...
		newC.push_back((1.0f-zero_num)*p1+zero_num*p2);
		nb_points_drawn++;
	}
	if(zero_den){ // it starts again here, or vice versa, it ends here
		...
		newC.push_back((1.0f-zero_den)*p1+zero_den*p2);
		nb_points_drawn++;
	}
	...
	this->scSegments.push_back(newC);
	if(m->pointcolors_.size()>0)
		this->scSegmentationIndices.push_back(m->pointcolors_[vec0]);
	this->scFaces.push_back(face);
